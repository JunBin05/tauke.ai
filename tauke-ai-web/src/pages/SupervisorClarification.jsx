        ...
        const loadSavedData = () => {
            try {
                ...
                if (finTrendStr) {
                    ...
                    // Force them into numbers so we don't hit null errors
                    const prevRevenue = Number(finTrend.previous_month_revenue) || 0;
                    const currRevenue = Number(finTrend.current_month_revenue) || 0;

                    if (prevRevenue > 0 && currRevenue > 0) {
                        // We have both months! Calculate the delta.
                        ...
                        // We only have this month's data (e.g., they didn't upload March)
                ...
                if (diagPatternsStr) {
                    ...
                    // Look for various ways the LLM might have named the top items
                    const topItems = diagPatterns.top_selling_items || diagPatterns.declining_items || [];

                    if (Array.isArray(topItems) && topItems.length > 0) {
                        setExternalSignal(`Top performing item: "${topItems[0]}" — monitor stock levels to prevent a supply gap.`);
                    } else if (diagPatterns.alert) {
                        setExternalSignal(diagPatterns.alert);
                    } else {
                        // 👇 THE CRITICAL FIX: If the LLM didn't find specific items, show a default safe message!
